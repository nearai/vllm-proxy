...
API_BASE = "https://f50b7a11efd86e51bd3a190fa851a0e329ad5cd9-3000.cvm1.near.ai:9204/"
...
def check_gpu(attestation, request_nonce):
    ...
    jwt_token = body[0][1]
    verdict = json.loads(base64url_decode_jwt_payload(jwt_token))[
...
def check_tdx_quote(attestation):
    ...
    intel_result = requests.post(
        PHALA_TDX_VERIFIER_API, json={"hex": attestation["intel_quote"]}, timeout=30
    ).json()
...
def show_compose(attestation, intel_result):
    ...
    print(
        "mr_config matches compose hash:",
        mr_config.lower().startswith(expected_mr_config.lower()),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Verify Phala Cloud TEE Attestation")
    parser.add_argument("--model", default="/deepseek-chat-v3-0324")
